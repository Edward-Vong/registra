...
export default function ReverseSearch() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await fetch('http://127.0.0.1:5000/reverse-search', {
        method: 'POST',
        headers: {
          ...
          {result && !loading && (
            <>
              {result.image_results?.length > 0 ? (
                <>
                  <div className="result-status-flagged">
                    ⚠ {result.image_results.length} match(es) found online
                  </div>
                  <div className="result-flagged-box">
                    ...
                    <div className="result-box-text">
                      Matching images appear elsewhere online. Review the links below to see whether the uses are authorized.
                    </div>
                ...
                <>
                  ...
                  <div className="result-box">
                    <div className="result-box-title">Your image looks clear</div>
                    <div className="result-box-text">
                      No matching images were found online in this search. That does not guarantee the image has never been reposted, but no direct matches were detected here.
                    </div>
                  </div>
                </>
              )}
